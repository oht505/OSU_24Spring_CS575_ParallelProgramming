...
main( int argc, char *argv[ ] )
{
	...
	PPSize    = NUMELEMENTS / NumCpus;		// per-processor data size -- assuming it comes out evenly 
	...
	// have the THEBOSS send to itself (really not a "send", just a copy):
	...
	if( me == THEBOSS )
	{
		...
		{
			BigSums[s] = PPSums[s];		// start the overall sums with the THEBOSS's sums
		}
	}
	else
	{
	...
	//fprintf( stderr, "Finish work, all done\n");
